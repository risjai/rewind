...
| **Bring your own framework** | Three-tier integration guide for agents not on the native list — pick `intercept.install()` (HTTP), `ExplicitClient` (custom transport), or ship a small connector for an org / framework family. ~5–50 LOC instead of writing a custom HTTP emitter per app. | [hdk.md](docs/hdk.md) | - |
...
| **Bring your own** | Custom agent / framework / org connector | ~5-50 LOC via [hdk.md](docs/hdk.md) — `intercept.install()` for HTTP, `ExplicitClient` for custom transport, small connector for org-wide defaults. |
